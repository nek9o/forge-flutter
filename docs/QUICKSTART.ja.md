...
> [!IMPORTANT]
> 本ガイドは **StabilityMatrix** から **Stable Diffusion WebUI Forge (Forge Neo)** をインストールし、**API 経由** で接続する構成を前提としています。
...
- StabilityMatrix 経由で **Forge Neo** がインストール済みであること
- 何らかの **Stable Diffusion モデル（.safetensors）** がダウンロード済みであること

---

## 1. Forge Neo の API を有効にする
...
2. Forge Neo のパッケージを見つけ、**歯車アイコン（起動オプション）** をクリック
3. 「追加の引数」セクションで **`--api`** オプションを有効にする
   - チェックボックスがある場合はオンにする
   - 手動入力欄がある場合は `--api` と入力する
4. 設定を **保存** する
5. Forge Neo を **起動**（または再起動）する

### APIの動作確認

Forge Neo が起動したら、ブラウザで以下の URL にアクセスしてください：
...
> [!TIP]
> StabilityMatrix のデフォルトでは Forge Neo は **ポート 7860** で起動します。
...
| Forge Neo の標準ポート | `http://127.0.0.1:7860` |

> [!WARNING]
> アプリのデフォルト値（ポート `7861`）と Forge Neo の標準ポート（`7860`）が異なる場合があります。
> Forge Neo の起動ログに表示されるポート番号を確認し、必要に応じて API URL を修正してください。
...
| 確認事項                     | 対処法                                             |
| ---------------------------- | -------------------------------------------------- |
| Forge Neo は起動しているか？ | StabilityMatrix で起動状態を確認                   |
...
- Forge Neo が完全に起動するまで待ってから、設定パネルを再読み込みしてください
- モデルファイル（`.safetensors`）が StabilityMatrix の models ディレクトリに配置されているか確認してください

### 画像が生成されない

- Forge Neo のコンソールにエラーが出ていないか確認してください
