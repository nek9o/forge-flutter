...
Desktop Client for Forge を使い始めるための手順を説明します。
...
Desktop Client for Forge は Forge の API を使って通信するため、Forge 側で API を有効にする必要があります。
...
## 2. Desktop Client for Forge を取得する
...
| 項目                           | 値                      |
| ------------------------------ | ----------------------- |
| デフォルト値                   | `http://127.0.0.1:7860` |
| Forge Classic Neo の標準ポート | `http://127.0.0.1:7860` |

> [!WARNING]
> アプリのデフォルト値（ポート `7860`）は Forge Classic Neo の標準ポートと同じですが、環境によって異なる場合があります。
